...
def generate_trade_url(item):
    ...
    def trans_merchant_order_name(the_record_dict):
        ...
        tmp_merchant_order_name_unicode = (
            "MERCHANT_ORDER_NAME%22%3A%22" + merchant_order_name + "%22%2C%22"
        )
    ...
    encode_trading_record = re.sub(
        "SEQNO", convert_merchant_order_name, encode_trading_record, 1
    )
    encode_trading_record = re.sub(
        "THUUID", convert_js_status, encode_trading_record, 1
    )
